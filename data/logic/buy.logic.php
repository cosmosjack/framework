...
class buyLogic {
     ...
     */
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id) { // cart_id = 1091|1

        //得到购买商品信息
        if ($ifcart) {// 判断是否是购物车结算
            $result = $this->getCartList($cart_id, $member_id);// 购物车
        } else {
            $result = $this->getGoodsList($cart_id, $member_id, $store_id); //非购物车
        }
...
//            ajaxReturn($freight_list,"JSON");
//        die;
        //算运费 // $freight_list   "iscalced":{"1":0},"nocalced":[]  299
        $store_freight_list = $this->_logic_buy_1->calcStoreFreight($freight_list, $city_id); // 1=>0
//        ajaxReturn($store_freight_list,'JSON');
//        die;
        $data = array();
        $data['state'] = empty($store_freight_list) ? 'fail' : 'success';
        $data['content'] = $store_freight_list; //{1:0}
...
//        P($input_buy_items); // array('76'=>1)
//        P($this->_post_data); // POST传递进来的数据
//        die;
        ...
        $order_pay_id = $model_order->addOrderPay($order_pay); // 订单支付记录
        if (!$order_pay_id) {
...
//        $store_cart_list = is_array($store_cart_list) ? $store_cart_list : array();
//        P($store_cart_list);
//        die;
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            /* 加入厂家信息 start */
            $order['is_supplier'] = $this->_post_data['is_supplier'] ? $this->_post_data['is_supplier']: '0';
            $order['by_staff'] = $this->_post_data['by_staff'] ? $this->_post_data['by_staff']: '0';
            /* 加入厂家信息 end */
//            P($order);
