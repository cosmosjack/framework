...
class buy_1Logic {
     ...
     */
    public function calcStoreFreight($freight_list, $city_id) {  //"iscalced":{"1":0},"nocalced":[]   299
...
//		return mt_rand(10,99)
		     return  "SC_". sprintf('%010d',time() - 946656000)
		      . sprintf('%03d', (float) microtime() * 1000)
		      . sprintf('%02d', (int) $member_id % 1000);
	}
...
//            $store_id_array = is_array($store_id_array) ? $store_id_array : array();
            foreach ($store_id_array as $store_id) {
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        /* 判断是否是商家用户  当前商品是否是厂家商品 是否存在关系 start */
        /*P($_SESSION['store_id']);
        P($goods_info['is_supplier']);*/
        if(isset($_SESSION['store_id']) && !empty($_SESSION['store_id'])){
            if($goods_info['is_supplier'] >0){
                $supplier_agent = new Model('supplier_agent');
                $data_supplier_agent = $supplier_agent->where(array('supplier_id'=>$goods_info['store_id'],'store_id'=>$_SESSION['store_id']))->find();
                if(!empty($data_supplier_agent['level']) && is_array($data_supplier_agent)){
                    // 如果存在关系 则求出 价格键值
                    switch($data_supplier_agent['level']){
                        case 1 :
                            $price_key = 'first_price';
                            break;
                        case 2 :
                            $price_key = 'second_price';
                            break;
                        case 3 :
                            $price_key = 'three_price';
                            break;
                        default :
                            $price_key = 'goods_price';
                            break;
                    }
                    $new_array['goods_price'] = $goods_info[$price_key];
                }else{
...
//        P($new_array['goods_price']);
        /* 判断是否是商家用户  当前商品是否是厂家商品 是否存在关系 end */

//        $new_array['goods_price'] = $goods_info['goods_price']; // 原来的价格
