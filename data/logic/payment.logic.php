...
class paymentLogic {
    ...
    public function getRealOrderInfo($pay_sn, $member_id = null) {
        ...
        $condition = array();
        $condition['pay_sn'] = $pay_sn;
        $order_list = $model_order->getNormalOrderList($condition); // 获取此订单的详细信息
     ...
     */
    public function getPaymentInfo($payment_code) {  // 3
        ...
        $inc_file = $_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR."shop".DS.'api'.DS.'payment'.DS.$payment_info['payment_code'].DS.$payment_info['payment_code'].'.php';
    ...
    public function updatePdOrder($out_trade_no,$trade_no,$payment_info,$recharge_info) {
        // $recharge_info 是获得的充值订单信息
        $log_path = BASE_DATA_PATH.DS."log".DS.'CZ.txt';
        $str = var_export($recharge_info,true);
        ...
        try {
			...
			if (intval($pdnum)>0) {
                @file_put_contents($log_path,$pdnum.time().PHP_EOL);
                throw new Exception('订单已经处理');
            }
            //更改充值状态
            $state = $model_pd->editPdRecharge($update,$condition);
            if (!$state) {
                @file_put_contents($log_path,array_merge($update,$condition).time().PHP_EOL);
