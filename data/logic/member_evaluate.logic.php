...
//        ajaxReturn($order_info,'JSON');
//        die;
        if (empty($order_info) || !$order_info['evaluate_able']){
            return callback(false, '订单已经被评论过');
        }
    ...
    public function save($date, $order_info, $store_info, $order_goods, $member_id, $member_name) {
        ...
        $result_evaluate = $model_evaluate_goods->addEvaluateGoodsArray($evaluate_goods_array, $goodsid_array);
        /*ajaxReturn(array_merge(array($result_evaluate),$evaluate_goods_array,$goodsid_array),"JSON");
        die;*/
        //添加店铺评价
