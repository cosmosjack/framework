 ...
 */
function GetMemberTotal($table_name){
    $db = new Dpdo();
 ...
 * $selectName 默认为1
 */
function allselect($dbname,$tabname,$id,$defaultKey,$where,$selectname=0){
    $db = Model($dbname);
    $defaultKey = $defaultKey ? $defaultKey : 'id';
    $html = '<select name="'.$dbname.'['.$selectname.'][]" class="inputselect">';
    $html.= '<option value="0">==钓友天下==</option>';

    $data = $db->where($where)->select();
    //转换数组

    foreach($data as $key => $var){
        $selected = $var[$defaultKey]==$id ? 'selected' : "";
        $html.='<option '.$selected.' value="'.$var[$defaultKey].'">'.$var[$tabname].'</option>';
        //$arr[$key] = array($var['id']=>$var[$tabname]);
    }
    $html.='</select>';
    return $html;
    //P($arr);
...
function getSscCode(){
    ...
    $url = 'http://www.lecai.com/lottery/draw/list/200';

    $handle = fopen("http://trend.caipiao.163.com/cqssc", "rb");
    $contents = stream_get_contents($handle);
    fclose($handle);
//echo $contents;
    $action_url = "http://33hao.net/index.php?act=cart&op=test";

//$pattern = '/<tbody(.*)/';
    $pattern = '/<tr\s+data-period(.*)/';
    preg_match_all($pattern,$contents,$match);
    /*echo '<pre>';
    print_r($match);
    echo '</pre>';*/
    $returnString = end($match[1]);
    preg_match_all('/"(.*)"\s/',$returnString,$matchPeriod);
    preg_match_all('/data-award="(.*)"/',$returnString,$matchAwad);
    /*echo '<pre>';
    print_r($matchPeriod);
    echo '</pre>';*/

//print_r($matchAwad);
    $period = $matchPeriod[1][0];
    $awad = str_replace(array(" ","　","\t","\n","\r"),"",$matchAwad[1][0]);
   // print($period);
  //  echo $awad;

    return array('period'=>$period,'awad'=>$awad);
}
/**
 * 产生验证码
 *
 ...
 * @return obj 对象形式的返回结果
 */
function Logic($model = null, $base_path = null){  // payment
    static $_cache = array();
    $cache_key = $model.'.'.$base_path;  //payment
    if (!is_null($model) && isset($_cache[$cache_key])) return $_cache[$cache_key];
    $base_path = $base_path == null ? BASE_DATA_PATH : $base_path;
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('InCosmos') or exit('Access Invalid!'); return ".$data.";";
 ...
 */
function findPayState($pay_sn,$model='order'){
        $db_order = new Model($model);
        $order_sum = $db_order->where(array('pay_sn'=>$pay_sn))->sum('order_amount');
        //P($order_sum);
        $MD5Info = getSignature('200011',$pay_sn,'','','C!nftXxo');
        $search_url = 'https://query.95epay.cn/searchInterfaceSingleFR.action?MerNo=200011&BillNo='.$pay_sn.'&MD5Info='.$MD5Info;
       // P($search_url);
        $curl = curl_init();
       ...
       // curl_setopt($curl, CURLOPT_HEADER, 1);
        ...
        $orderInfo = $data->MerInfo;
        $num = count($orderInfo->OrderInfo);
        $all_amount = 0.00;
        for($i=0;$i<$num;$i++){
           // P($orderInfo->OrderInfo[$i]);
            if($orderInfo->OrderInfo[$i]['State'] == 1){
                //echo '已经支付';
                $all_amount = floatval($orderInfo->OrderInfo[$i]['amount']) + floatval($all_amount);
               // echo $orderInfo->OrderInfo[$i]['amount'];
               // echo $orderInfo->OrderInfo[$i]['Date'];
                $amount_date = $orderInfo->OrderInfo[$i]['Date'];
            }
        }
         $already_pay = floatval($all_amount);

    if($already_pay >= $order_sum){
       // echo '已经支付完成';
        return array('state'=>1,'msg'=>"已经付款",'amount_date'=>strtotime($amount_date));
    }else{
       // echo '已经支付'.$already_pay;
        return array('state'=>2,'msg'=>"未付款",'needpay'=>$already_pay);
    }
