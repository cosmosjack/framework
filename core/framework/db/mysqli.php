 ...
 *
 *
 * @package    db* cosmos-jack为你提供售后服务 以便你更好的了解
 */
...
//         if (is_object(self::$link[$host]) && !self::$link[$host]->ping()) {
	 ...
	 * @return mixed
	 */
	public static function query($sql, $host = 'master',$mothed){
		self::connect($host);
	...
	public static function getCount($table, $condition = null, $host = 'slave'){
		...
		if (!empty($condition) && is_array($condition)){
			...
			foreach ($condition as $key=>$val) {
                ...
                /* 如果$val是其他非等号 判断 start */
                    if(!empty($val) && is_array($val)){

                            $condition_key = strtoupper($val[0]);
//                            echo $condition_key;
                            self::parseKey($key);
                            $val[1] = self::parseValue($val[1]);
                            switch($condition_key){
                                case 'LIKE':
                                    $where .= ' AND '.$key.' LIKE '.$val[1];
                                    break;
                                case 'GT':
                                    $where .= '';
                                    break;
...
//        die;
	 ...
	 * @return
	 */
	public static function execute($sql, $host = 'master',$mothed=''){

		self::connect($host);
		$result = self::query($sql,$host,$mothed);
		return $result;
	}
