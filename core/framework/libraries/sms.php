 ...
 *
 *
 * @package    library* cosmos-jack为你提供售后服务 以便你更好的了解
 */
...
class Sms {
    ...
    */
    public function send($mobile,$content) {
        ...
        {
            return $this->mysend_lingkai($mobile,$content);
        }
	...
	*/
    ...
    private function mysend_lingkai($mobile,$content) {
        $user_id = urlencode(C('mobile_username')); // 这里填写用户名
        $pass = urlencode(C('mobile_pwd')); // 这里填登陆密码
        if(!$mobile || !$content || !$user_id || !$pass) return false;
        if(is_array($mobile)) $mobile = implode(",",$mobile);
        $mobile=urlencode($mobile);
        //$content=$content."【我的网站】";
        $content=$this->utf8_to_gbk($content);
        $content=urlencode($content);
        //$pass =md5($pass);//MD5加密
        //$url="http://api.smsbao.com/sms?u=".$user_id."&p=".$pass."&m=".$mobile."&c=".$content."";
        $url="https://sdk2.028lk.com/sdk2/BatchSend2.aspx?CorpID=".$user_id."&Pwd=".$pass."&Mobile=".$mobile."&Content=".$content."&SendTime=";
        $res = file_get_contents($url);
        // return $res;
        /*
    // $ch = curl_init();
