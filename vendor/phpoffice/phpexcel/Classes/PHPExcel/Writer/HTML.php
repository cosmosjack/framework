...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		// .gridlines td { }
		$css['.gridlines td']['border'] = '1px solid black';
		...
		// .n {}
		$css['.n']['text-align'] = 'center'; // NUMERIC

		// .s {}
		$css['.s']['text-align'] = 'center'; // STRING

		// Calculate cell style hashes
		foreach ($this->_phpExcel->getCellXfCollection() as $index => $style) {
//			$css['td.style' . $index] = $this->_createCSSStyle( $style ); // 原来的
//			$css['th.style' . $index] = $this->_createCSSStyle( $style );   // 原来的
            $css['td.style0'] = $this->_createCSSStyle( $style );
            $css['th.style0'] = $this->_createCSSStyle( $style );
		}
