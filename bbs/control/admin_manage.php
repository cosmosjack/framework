 ...
 * QQ:  997823131 
 */
class admin_manageControl extends BaseAdminControl{
    private $links = array(
        array('url'=>'act=admin_manage&op=admin','lang'=>'limit_admin'),
        array('url'=>'act=admin_manage&op=admin_add','lang'=>'admin_add_limit_admin'),
        array('url'=>'act=admin_manage&op=gadmin','lang'=>'limit_gadmin'),
        array('url'=>'act=admin_manage&op=gadmin_add','lang'=>'admin_add_limit_gadmin'),
    );
    ...
    public function admin_addOp(){
        if (chksubmit()){
            ...
            if ($rs){
                ...
                showMessage(L('nc_common_save_succ'),'index.php?act=admin_manage&op=admin');
            }else {
                showMessage(L('nc_common_save_fail'));
            }
        }
    ...
    public function gadmin_setOp(){
        ...
        if (chksubmit()){
            ...
            if ($update){
                ...
                showMessage(L('nc_common_save_succ'),'index.php?act=admin_manage&op=gadmin');
            }else {
    ...
    public function admin_editOp(){
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage(Language::get('admin_edit_success'),'index.php?act=admin_manage&op=admin');
            }else{
                showMessage(Language::get('admin_edit_fail'),'index.php?act=admin_manage&op=admin');
            }
        }else{
            //查询用户信息
            $admin_model = Model('admin');
            ...
            if (!is_array($admininfo) || count($admininfo)<=0){
                showMessage(Language::get('admin_edit_admin_error'),'index.php?act=admin_manage&op=admin');
            }
