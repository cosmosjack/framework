<?php defined('InCosmos') or exit('Access Invalid!');?>
<!--我的收藏-->
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8" />
	<title>好少年</title>
	<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no" />
</head>
    <link rel="stylesheet" type="text/css" href="<?php echo BBS_RESOURCE_SITE_URL;?>/bootstrap/css/tpl/collect.css" />
<body>
    <div class="collect">
    	<!--头部-->
    	<div class="reg_top oveflow">
    		<button type="button" class="btn oprev">
	          <span class="glyphicon glyphicon-chevron-left"></span>
	        </button>
	        <span class="col-xs-12 text-center">我的收藏</span>
    	</div>
    	<div class="content">
    		<div class="index_pro">
	    	 	<div class="overflow index_pro_Img">
	    	 		<img src="<?php echo BBS_RESOURCE_SITE_URL;?>/bootstrap/img/img0.jpg" class="pro_img" />
	                <!-- <div class="col-xs-10 col-xs-offset-1 Prompt text-right"><span>已经满额<span></div> -->
	    	 		<div class="mask">
	    	 			<span class="pull-left">惠州市东湖西路5号 6-13岁</span>
	    	 			<span class="pull-left text-right">亲子活动、成长</span>
	    	 		</div>
	    	 	</div>
	    	 	<div class="overflow pro_tit">
	    	 		<div class="col-xs-10 col-xs-offset-1">
	    	 			<span class="pull-left col-xs-8 ProTitle">
	    	 				<span class="city">【惠州】</span>宝贝加油，亲子互动
	    	 			</span>
	    	 			<span class="people pull-right text-right">
	    	 				已参与 
	    	 				<span class="past">13</span>/<span class="sum">20</span>
	    	 			</span>
	    	 		</div>
	    	 	</div>
	    	 	<div class="overflow time_price">
	    	 		<div class="col-xs-10 col-xs-offset-1">
	    	 			<div class="pull-left">
	    	 				<p>3月15日一天</p>
	                        <p class="price">&yen;360</p>
	    	 			</div>
	    	 			<div class="overflow pull-left text-right">
	    	 				<button class="pro_btn">去订票</button>
	    	 			</div>
	    	 		</div>
	    	 	</div>
    	    </div>
    	    <div class="index_pro">
	    	 	<div class="overflow index_pro_Img">
	    	 		<img src="<?php echo BBS_RESOURCE_SITE_URL;?>/bootstrap/img/children.jpg" class="pro_img" />
	                <div class="col-xs-10 col-xs-offset-1 Prompt text-right"><span>已经满额<span></div> 
	    	 		<div class="mask">
	    	 			<span class="pull-left">惠州市东湖西路5号 6-13岁</span>
	    	 			<span class="pull-left text-right">亲子活动、成长</span>
	    	 		</div>
	    	 	</div>
	    	 	<div class="overflow pro_tit">
	    	 		<div class="col-xs-10 col-xs-offset-1">
	    	 			<span class="pull-left col-xs-8 ProTitle">
	    	 				<span class="city">【惠州】</span>宝贝加油，亲子互动
	    	 			</span>
	    	 			<span class="people pull-right text-right">
	    	 				已参与 
	    	 				<span class="past">13</span>/<span class="sum">20</span>
	    	 			</span>
	    	 		</div>
	    	 	</div>
	    	 	<div class="overflow time_price">
	    	 		<div class="col-xs-10 col-xs-offset-1">
	    	 			<div class="pull-left">
	    	 				<p>3月15日一天</p>
	                        <p class="price">&yen;360</p>
	    	 			</div>
	    	 			<div class="overflow pull-left text-right">
	    	 				<button class="pro_btn btn_isabled" disabled="disabled">已售罄</button>
	    	 			</div>
	    	 		</div>
	    	 	</div>
    	    </div>
    	    <div class="index_pro">
	    	 	<div class="overflow index_pro_Img">
	    	 		<img src="<?php echo BBS_RESOURCE_SITE_URL;?>/bootstrap/img/img0.jpg" class="pro_img" />
	                <div class="col-xs-10 col-xs-offset-1 Prompt text-right"><span>已经满额<span></div> 
	    	 		<div class="mask">
	    	 			<span class="pull-left">惠州市东湖西路5号 6-13岁</span>
	    	 			<span class="pull-left text-right">亲子活动、成长</span>
	    	 		</div>
	    	 	</div>
	    	 	<div class="overflow pro_tit">
	    	 		<div class="col-xs-10 col-xs-offset-1">
	    	 			<span class="pull-left col-xs-8 ProTitle">
	    	 				<span class="city">【惠州】</span>宝贝加油，亲子互动
	    	 			</span>
	    	 			<span class="people pull-right text-right">
	    	 				已参与 
	    	 				<span class="past">13</span>/<span class="sum">20</span>
	    	 			</span>
	    	 		</div>
	    	 	</div>
	    	 	<div class="overflow time_price">
	    	 		<div class="col-xs-10 col-xs-offset-1">
	    	 			<div class="pull-left">
	    	 				<p>3月15日一天</p>
	                        <p class="price">&yen;360</p>
	    	 			</div>
	    	 			<div class="overflow pull-left text-right">
	    	 				<button class="pro_btn btn_isabled" disabled="disabled">已售罄</button>
	    	 			</div>
	    	 		</div>
	    	 	</div>
    	    </div>
    	</div>
    </div>
    <script type="text/javascript" src="<?php echo BBS_RESOURCE_SITE_URL;?>/bootstrap/js/tpl/collect.js"></script>
</body>
</html>